...
export const EditExerciseModal: React.FC<EditExerciseModalProps> = ({ exercise, seriesId, onSave, onClose }) => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    // Generate snippet from the first line of the detailed correction.
      ...
      >
        ...
        <form onSubmit={handleSave} id="edit-exercise-form" className="flex-grow overflow-y-auto p-6 space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="statement" className="block text-sm font-medium text-gray-300 mb-1">Énoncé</label>
                 <MathKeyboard
                    initialValue={formData.statement}
                    onExpressionChange={(latex) => handleChange('statement', latex)}
                    showPreview={true}
                 />
              </div>
               <div>
                ...
                <MathKeyboard
                    initialValue={formData.fullCorrection}
                    onExpressionChange={(latex) => handleChange('fullCorrection', latex)}
                    showPreview={true}
                    placeholder="Détailler chaque étape de calcul ici. L'extrait de correction sera généré à partir de la première ligne."
                 />
              </div>
              <div>
            ...
            {/* Right column for formula and preview */}
            <div className="space-y-4">
              <div>
        ...
        <footer className="flex justify-end gap-4 p-4 border-t border-gray-700 bg-gray-800/50 flex-shrink-0">
          ...
          >
            {isCreating ? 'Ajouter l\'exercice' : 'Enregistrer les modifications'}
          </button>
        </footer>
      </div>
    </div>
