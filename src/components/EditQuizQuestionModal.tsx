...
interface EditQuizQuestionModalProps {
  ...
  quizId: string;
  chapterId: string;
  onSave: (questionData: QuizQuestion, quizId: string, chapterId: string) => void;
  onClose: () => void;
}
...
export const EditQuizQuestionModal: React.FC<EditQuizQuestionModalProps> = ({ question, quizId, chapterId, onSave, onClose }) => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    onSave(finalQuestion, quizId, chapterId);
    onClose();
  };
    ...
    >
      ...
      >
        ...
        <footer className="flex justify-end gap-4 p-4 border-t border-gray-700 bg-gray-800/50">
          <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-semibold rounded-lg transition-colors duration-200 bg-gray-700/50 border-2 border-gray-600 hover:bg-gray-700 hover:border-gray-500 text-gray-300">
            Annuler
          </button>
          <button type="submit" form="edit-question-form" className="px-4 py-2 text-sm font-semibold rounded-lg transition-colors duration-200 bg-brand-blue-600 border-2 border-brand-blue-500 text-white hover:bg-brand-blue-700">
            {isCreating ? 'Ajouter la question' : 'Enregistrer'}
          </button>
        </footer>
      </div>
    </div>
